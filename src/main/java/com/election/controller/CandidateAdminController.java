...
public String addCandidate(@RequestBody Candidatebean candidatebean) {
	...
	for(Electionbean fd: fdetails) {
		...
		((Partybean) candidatebean.getParty()).setPartyID(rd.getPartyID());
...
public void setAdms(AdminService adms) {
	this.adms = adms;
}

public ElectionDetailsRepo getFl() {
	return fl;
}

public void setFl(ElectionDetailsRepo fl) {
	this.fl = fl;
}

public PartyDetailsRepo getRpo() {
	return rpo;
}

public void setRpo(PartyDetailsRepo rpo) {
	this.rpo = rpo;
}
